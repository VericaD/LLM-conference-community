...
def summarize_token_usage(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "prompt_tokens": safe_sum("prompt_tokens"),
        "completion_tokens": safe_sum("completion_tokens"),
        "total_tokens": safe_sum("total_tokens"),
        "prompt_chars": safe_sum("prompt_chars"),
        "completion_chars": safe_sum("completion_chars"),
    }
...
def build_prompt(paper_id: str, paper_text: str, reviewer_model: str) -> str:
    ...
    Only explicit, concrete, and verifiable evidence from the paper should influence scores.

    If information is not explicitly stated, treat it as missing.
    Do NOT infer missing details.
    Do NOT reward fluency, confidence, scientific tone, or technical jargon.

    Concrete specificity is more important than impressive wording.

    Before scoring:
    1. Extract explicit evidence from the paper
    2. Identify missing or vague information
    3. Evaluate criteria independently
    4. Apply penalties where flaws meaningfully affect quality
    5. Compute final dimension scores consistently
    ...
    Combining existing techniques or using new terminology alone is NOT sufficient for high novelty.
    ...
    2. method_clearly_explained | dimension=clarity | weight=4
    ...
    - score = 0.5:
    The flaw is partially present and meaningfully affects the paper.
    ...
    CRITICAL RULE:
    Penalty scores should reflect severity independently.

    A partially satisfied positive criterion does NOT automatically imply a penalty.
    Assign penalty = 0.5 only if the flaw meaningfully affects the paper.
    ...
    Avoid inconsistencies where a major flaw is ignored.
    ...
    - satisfied (score = 1.0):
    Requires concrete, specific, detailed, and verifiable evidence.

    - partially_satisfied (score = 0.5):
    Evidence exists but is incomplete, vague, high-level, or insufficiently detailed.

    - not_satisfied (score = 0.0):
    Criterion is unsupported, extremely vague, superficial, or lacks necessary technical detail.

    CRITICAL RULE:
    Mentioning something is NOT enough.

    Claims without quantitative, methodological, or comparative detail should NOT be treated as strong evidence.

    When in doubt:
    - If there is weak but real evidence → partially_satisfied
    - If evidence is minimal, vague, superficial, or mostly rhetorical → not_satisfied
    ...
    Scores of 5 should be rare and require exceptional evidence across the dimension.
    ...
    Avoid clustering all scores in the 2–3 range.

    - Scores of 4 require clearly strong evidence
    - Scores of 5 require exceptional and rare quality
    - Scores of 1 should be used when content is largely missing or invalid

    ----------------------------------------
    DISCRIMINATION REQUIREMENT
    ----------------------------------------

    You must differentiate between weak, average, and strong papers.

    Avoid assigning identical scores across all dimensions unless strongly justified.

    At least one dimension should reflect the paper’s strongest aspect.
    At least one dimension should reflect its weakest aspect.
    ...
    - Did fluent writing or technical jargon influence scoring without concrete evidence?
    ...
    Only if the paper provides detailed, concrete, reproducible, and well-supported information across method and experiments.

    - 4 = reasonably high confidence
    Minor missing details, but overall evaluation is reliable.

    - 3 = moderate confidence
    Some important aspects are unclear, vague, or underspecified.
    ...
    If more than 50% of criteria are partially_satisfied → confidence should be <= 3.
        ...
        try:
            ...
            token_usage.append(
                make_token_record(
                    response=response,
                    attempt=attempt,
                    call_name="analytical_review",
                    prompt=retry_prompt,
                    output_text=raw,
                )
            )

            result = validate_and_patch_output(
                raw_json=raw,
                paper=paper,
                paper_id=paper_id,
                model=model,
            ...
            result["token_usage_summary"] = summarize_token_usage(token_usage)
...
def main() -> None:
    ...
    parser.add_argument("--think", type=str, default=None, help="false, true, low, medium, high, or none")
    ...
    run_token_usage: list[dict[str, Any]] = []
    ...
    for idx, paper_path in enumerate(all_files, start=1):
        ...
        try:
            ...
            run_token_usage.extend(result.get("token_usage", []))
            succeeded += 1
    ...
    summary = {
        ...
        "token_usage_summary": summarize_token_usage(run_token_usage),
    }
