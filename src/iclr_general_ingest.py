#!/usr/bin/env python3
"""
Ingest ICLR conference data from OpenReview into JSONL and SQLite.
The script retrieves submissions and their associated notes, normalizes reviews
and decisions, downloads PDFs and extract their text and report
summary statistics about the collected dataset.
...
def build_iclr_config(year: int, venue_id_override: Optional[str] = None) -> VenueConfig:
    """
    Build a generic config for ICLR conference years.

    The exact submission invitation may vary across years. It is resolved later
    by checking common invitation names and probing likely candidates.
    """
    venue_id = venue_id_override or f"ICLR.cc/{year}/Conference"
...
def download_pdf_bytes(pdf_url: str, timeout_s: int = 60, max_retries: int = 5, base_sleep_s: float = 3.0,) -> bytes:
    headers = {
        "User-Agent": (
    ...
    for attempt in range(max_retries):
        try:
            ...
            if r.status_code == 429:
                ...
                print(f"[pdf] rate limited; sleeping {sleep_s:.1f}s before retry")
                time.sleep(sleep_s)
                continue
            ...
            if attempt < max_retries - 1:
                sleep_s = base_sleep_s * (2 ** attempt)
                print(f"[pdf] download failed; sleeping {sleep_s:.1f}s before retry: {e}")
                time.sleep(sleep_s)
...
CREATE TABLE IF NOT EXISTS notes (
  ...
  edition_id TEXT NOT NULL REFERENCES editions(edition_id),
  forum      TEXT NOT NULL REFERENCES papers(forum) ON DELETE CASCADE,
  replyto    TEXT,
...
CREATE TABLE IF NOT EXISTS reviews (
  review_id   TEXT PRIMARY KEY REFERENCES notes(note_id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_reviews_forum ON reviews(forum);
...
CREATE TABLE IF NOT EXISTS paper_decisions (
  ...
  forum       TEXT NOT NULL REFERENCES papers(forum) ON DELETE CASCADE,
  edition_id  TEXT NOT NULL REFERENCES editions(edition_id),
  decision_note_id TEXT REFERENCES notes(note_id) ON DELETE SET NULL,
  decision    TEXT,
  decision_text TEXT,
  raw_json    TEXT NOT NULL
);

CREATE INDEX IF NOT EXISTS idx_paper_decisions_forum ON paper_decisions(forum);
CREATE INDEX IF NOT EXISTS idx_paper_decisions_decision ON paper_decisions(decision);
"""
...
def invitation_has_notes(client: "openreview.Client", invitation_id: str, api_version: int) -> bool:
    try:
        notes = client.get_notes(invitation=invitation_id, limit=1)
        return bool(notes)
    except Exception:
        return False
...
def resolve_submission_invitation(client: "openreview.Client", config: VenueConfig, api_version) -> str:
    ...
    # Some OpenReview years do not expose the main submission invitation
    # in the invitation list, but it can still be queried directly
    for suffix in config.submission_invitation_candidates:
        candidate = f"{config.venue_id}/-/{suffix}"
        if invitation_has_notes(client, candidate, api_version):
            return candidate
...
def get_content_value(content: Dict[str, Any], *keys: str) -> Optional[Any]:
 
    if not isinstance(content, dict):
        return None

    normalized_content = {
        normalize_key(str(k)): unwrap_api2_content_value(v)
        for k, v in content.items()
    }

    for key in keys:
        value = normalized_content.get(normalize_key(key))
...
def get_content_text(content: Dict[str, Any], *keys: str) -> Optional[str]:
    value = get_content_value(content, *keys)
    if value is None:
        return None
...
def get_paper_id_for_forum(conn: sqlite3.Connection, forum: str) -> Optional[str]:
    cur = conn.execute(
        "SELECT paper_id FROM papers WHERE forum = ?",
        (forum,),
    )
    row = cur.fetchone()
    return row[0] if row else None

def insert_review(conn: sqlite3.Connection, note: Dict[str, Any]) -> None:
    content = note.get("content") or {}
    signatures = note.get("signatures") or []

    paper_id = get_paper_id_for_forum(conn, note["forum"])
    if paper_id is None:
        return

    rating = get_content_value(content, "rating", "recommendation", "overall_recommendation")
    confidence = get_content_value(content, "confidence", "reviewer_confidence")

    conn.execute(
        """
        INSERT OR REPLACE INTO reviews(
          review_id, paper_id, forum, edition_id, reviewer_signature,
          rating, confidence, summary, strengths, weaknesses, questions,
          review_text, content_json, field_keys_json, raw_json
        ) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
        """,
        (
            note["note_id"],
            ...
            parse_numeric(rating),
            parse_numeric(confidence),
            get_content_text(content, "summary"),
            get_content_text(content, "strengths"),
            get_content_text(content, "weaknesses"),
            get_content_text(content, "questions"),
            get_content_text(
                content,
                "review",
                "review_text",
                "main_review",
                "comments",
                "comments_to_the_author",
            ),
            jdump(content),
            jdump([normalize_key(str(k)) for k in content.keys()]) if isinstance(content, dict) else "[]",
            jdump(note),
        ),
    )

def insert_paper_decision(conn: sqlite3.Connection, note: Dict[str, Any]) -> None:
    content = note.get("content") or {}

    forum = note.get("forum") or note["note_id"]
    paper_id = get_paper_id_for_forum(conn, forum)

    if paper_id is None and note.get("note_id"):
        paper_id = note["note_id"]

    if paper_id is None:
        return

    decision = get_content_text(content, "decision", "recommendation")
    decision_text = get_content_text(
        content,
        "comment",
        "comments",
        "decision_text",
    ...
    conn.execute(
        ...
        INSERT OR REPLACE INTO paper_decisions(
          paper_id, forum, edition_id, decision_note_id,
          decision, decision_text, raw_json
        ) VALUES(?,?,?,?,?,?,?)
        """,
        (
            paper_id,
...
def step_build_sqlite(
    ...
    for n in read_jsonl(notes_jsonl):
        ...
        if n.get("note_type") == "official_review":
            insert_review(conn, n)

        content = n.get("content") or {}

        if n.get("note_type") == "decision":
            insert_paper_decision(conn, n)

        elif n.get("note_type") != "official_review":
            has_decision_signal = (
                get_content_value(content, "decision", "recommendation") is not None
                and get_content_value(content, "metareview", "meta_review", "decision_text", "comment") is not None
            )
...
def step_quick_stats(db_path: str) -> None:
    ...
    cur.execute("SELECT COUNT(*) FROM paper_decisions")
...
def main() -> None:
    ...
    ap.add_argument("--year", type=int, required=True, help="ICLR year")
    ...
    ap.add_argument("--fetch-pdf-text", action="store_true", help="Download PDFs and extract plain text into SQLite")
    ap.add_argument("--pdf-limit", type=int, default=None, help="For testing, limit number of PDFs to process")
    ap.add_argument("--pdf-min-chars", type=int, default=500, help="Minimum extracted character count to consider PDF text extraction successful")
    ap.add_argument("--pdf-no-skip-existing", action="store_true", help="Reprocess all PDFs, including PDFs already successfully extracted")
