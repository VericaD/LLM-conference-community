#!/usr/bin/env python3
"""
RAG Indexing Layer — ICLR 2019 Sample
======================================
Reads a small sample of papers from the SQLite database produced by
ICLRgeneral_ingest.py, chunks them, embeds them with a local Ollama
embedding model, and stores the vectors in ChromaDB.

Usage:
    python rag_indexing.py --db path/to/iclr2019.sqlite --sample 20
    python rag_indexing.py --db path/to/iclr2019.sqlite --sample 20 --reset

Requirements:
    pip install chromadb ollama

Ollama model (pull once before running):
...
EMBED_MODEL = "nomic-embed-text"   # swap to "bge-m3" for stronger embeddings
CHROMA_DIR  = "./chroma_iclr"      # where ChromaDB persists to disk
COLLECTION  = "iclr_papers"

# Chunking settings
ABSTRACT_MAX_CHARS   = 2000        # abstracts are usually < 1500 chars, so this is safe
PDF_CHUNK_SIZE       = 1200         # characters per PDF body chunk
PDF_CHUNK_OVERLAP    = 200         # overlap between consecutive chunks
...
def store_chunks(
    ...
    # ChromaDB accepts at most 5 461 items per upsert — batch to be safe
    batch_size = 500
    total = len(chunks)
    for i in range(0, total, batch_size):
        collection.upsert(
...
def sanity_check(collection: chromadb.Collection, query: str = "attention mechanism") -> None:
    """
    Run one test retrieval query and print the top-3 results.
    Lets you verify the index is working before moving on to generation.
    """
...
def main() -> None:
    ...
    ap.add_argument("--sample", type=int, default=20,
                    help="Number of papers to index (default: 20)")
    ...
    ap.add_argument("--sanity-query", default="attention mechanism",
                    help="Query string for the sanity check at the end")
    args = ap.parse_args()
    ...
    print("  Next step: use rag_generate.py to query it during paper generation.")
